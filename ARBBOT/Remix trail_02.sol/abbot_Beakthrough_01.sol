...
contract Arbbot is ReentrancyGuard, AccessControl {
  ...
  //uint256 public _flashLoanPremiumTotal; // Declare the variable
  uint public _flashLoanPremiumTotal = 9;
  ...
  address public constant WETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE; // Replace with actual WETH address 
 ...
 // Modifiers
  ...
  modifier onlyRole(bytes32 role) {
    require(userRoles[msg.sender] & role != 0, "Not authorized");
    _;
  }*/
  ...
  */
  ...
  function addToken(
    ...
    onlyRole(OWNER_ROLE)
  {
    tokens[token] = TokenInfo(name, token, symbol);  
  }
  ...
  */
  ...
  function _makeTrade(ArbInfo memory info) internal {
    // Define input amount
    uint amountIn = _getTokenInputAmount(info.amountOutMin, info.tokenOut);
  ...
  function _repayLoan(ArbInfo memory info) internal {
    ...
    transferToken(info.tokenIn, 0x987115C38Fd9Fd2aA2c6F1718451D167c13a3186, amountOwed);
  }
  ...
  function _getLoanAmount(ArbInfo memory info) internal returns (uint) {
    ...
    uint amountOut = _getTokenOutputAmount(amountIn, info.tokenIn, info.tokenOut);
  ...
  function getAmountIn(ArbInfo memory info, uint minProfit) internal returns (uint) {
    ...
    uint loanPremiumFee = getFlashloanFee(info.amountOutMin);
    uint swapExchangeFee =  getSwapFee(info);
    //uint gasFees = getArbitrageGasFees(info);
  ...
  /*function _estimateArbitrageGasFees(ArbInfo memory arbInfo) internal {
    ...
    return gas;
    //return (success, result, uint gas);
    //return estimateGasCost(address(this), data);
  }*/

  // Estimate gas fees for the arbitrage opportunity
  function _estimateArbitrageGasFees(ArbInfo memory info) internal returns (uint256) {
    // Encode arbitrage data (you need to define your specific data format)
    bytes memory arbitrageData = abi.encode(info.tokenIn, info.tokenOut, info.amountInMin, info.amountOutMin);

    // Call the external function with dynamic token addresses
    (bool success, bytes memory result, uint256 gas) = GasEstimator(address(this)).estimateGasCostForArbitrage(
      address(this),
      arbitrageData
    );
    require(success, "Estimation failed");
  ...
  */
  ...
  function getPrice(address token) public view returns (uint) {
    if (token == ETH_ADDRESS) {  
      (,,uint ethPrice,,) = ethUsdFeed.latestRoundData();
      return uint(ethPrice);
    } else {  
      // Get token/ETH price from Uniswap
      address pair = IUniswapV2Factory(factory).getPair(token, WETH_ADDRESS); 
      (uint reserve0, uint reserve1,) = IUniswapV2Pair(pair).getReserves();
      return reserve1/reserve0; // token/ETH price
    }
    if (token == DAI_ADDRESS) {
      (,,uint daiPrice,,) = daiUsdFeed.latestRoundData(); 
      return uint(daiPrice);
    } else {  
      // Get token/DAI price from Uniswap
      address pair = IUniswapV2Factory(factory).getPair(token, DAI_ADDRESS); 
      (uint reserve0, uint reserve1,) = IUniswapV2Pair(pair).getReserves();
      return reserve1/reserve0; // token/ETH price
    }
  ...
  function getSwapFee(ArbInfo memory info) public view returns (uint) {
    ...
    // Estimate the swap fee using the custom logic
    uint estimatedFee = estimateSwapFee(pair);

    return estimatedFee;
  }

  function estimateSwapFee(address pair) internal view returns (uint) {
    // Get the total liquidity in the pool
    uint totalLiquidity = IUniswapV2Pair(pair).totalSupply();

    // Assume a fixed fee rate (e.g., 0.30%)
    uint feeRate = 30; // 0.30% expressed as an integer

    // Calculate the estimated swap fee
    uint estimatedFee = (totalLiquidity * feeRate) / 10000; // Divide by 10000 to convert percentage to basis points
  ...
  */
    ...
    onlyRole(OWNER_ROLE | TRADER_ROLE | MANAGER_ROLE)
  {
    for (uint i = 0; i < opportunities.length; i++) {
      ...
      uint256 gasCost = _estimateArbitrageGasFees(info);
  ...
  function getArbitrageGasFees(ArbInfo memory info) public returns (uint256) {
    // Estimate the gas cost of a transaction
    uint gasPrice = getGasPrice();
    uint gasAmount = _estimateArbitrageGasFees(Info);
    return gasPrice * gasAmount;
  }
