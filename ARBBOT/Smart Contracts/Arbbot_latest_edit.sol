...
interface GasEstimatorCompatible {
  function estimate(
      address _to,
      bytes calldata _data
  ) external returns (bool success, bytes memory result, uint256 gas) {
...
//import "@beaconcha/gasnow/contracts/GasNow.sol";
...
contract ArbbotContractV1Salvation is ReentrancyGuard {
 ...
 // using SafeMath for uint;
  ...
  uint private accumulatedProfits;
  ...
  address public _lendingPool = 0x987115C38Fd9Fd2aA2c6F1718451D167c13a3186;
  ...
  GasEstimatorCompatible public gasEstimator;
 ...
 /* modifier onlyUserRole(string memory role) {
  ...
  constructor(address _priceFeed, address _uniswapRouter, address _lendingPool, address _profitAccount) {
    ...
    gasEstimator = GasEstimatorCompatible();
  }
  ...
  */
  ...
  function startArbitrage(address tokenIn, address tokenOut) external nonReentrant onlyRole(OWNER_ROLE || TRADER_ROLE) whenNotPaused {
    // Add and start arbitrage for the specified pairs
    TokenPair[] memory pairs = new TokenPair[](3);
    pairs[0] = TokenPair({
    tokenIn: [tokenIn, address(0), address(0)],
    tokenOut: [tokenOut, address(0), address(0)],
    locked: false
    });
    pairs[1] = TokenPair({
    tokenIn: [tokenIn, address(0), address(0)],
    ...
    bool addAndStartArbitragePairs; 
  ...
  function addAndStartArbitragePairs(TokenPair[] memory pairs) external nonReentrant onlyRole(OWNER_ROLE || TRADER_ROLE) whenNotPaused {
   ...
   function createArbBatch(bytes32[] calldata opportunityIds, 
    ...
    ) external nonReentrant onlyRole(OWNER_ROLE || TRADER_ROLE) 
    whenNotPaused {
  ...
  User[msg.sender].subscriptionExpiry = expiration;
  }
  ...
  */
  ...
  function createArbBatch(bytes32[] calldata opportunityIds, 
    address[3] calldata tokenInAddresses,
    address[3] calldata tokenOutAddresses
    ) internal nonReentrant onlyRole(OWNER_ROLE || TRADER_ROLE) 
    whenNotPaused {
    uint256 batchId = nextBatchId++;
    pendingBatches[batchId].batchOpportunities = opportunityIds;
    pendingBatches[batchId].tokenPairAddress = tokenInAddresses;
